...
export default function PartnerServicesPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        if (data.success) {
          setSuccessMsg('Service updated and submitted for approval.');
          setShowModal(false);
          fetchData();
        }
      } else {
        ...
        if (data.success) {
          setSuccessMsg('Service created and submitted for approval.');
          setShowModal(false);
          fetchData();
        }
      }
  ...
  // Filter potential parent services (only parent services created by this partner, excluding editing one)
  const potentialParents = services.filter(s => !s.parentId && s._id !== editingId);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between border-b border-gold/15 pb-4">
        <div>
          ...
          <p className="text-xs text-foreground/50 font-medium">Create and customize services for your clients</p>
        </div>
        <button 
      ...
      <div className="bg-white rounded-3xl border border-gold/20 shadow-sm overflow-hidden">
        {services.length === 0 ? (
          <div className="py-16 text-center text-foreground/40 text-sm">No services configured. Create your first service.</div>
        ) : (
