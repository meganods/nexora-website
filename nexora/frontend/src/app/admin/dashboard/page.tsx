...
import {
  ...
  Wallet, Bell, TrendingUp, ListCollapse, Menu, Eye, Globe, Map, MapPin, Hash, Building, LifeBuoy, Receipt, MessageSquare, LogOut, Send
} from 'lucide-react';
...
function AdminDashboardContent() {
  ...
  const updateContactMessageStatus = async (id: string, status: string) => {
    try {
      ...
      if (selectedContactMsg?._id === id) setSelectedContactMsg((p: any) => ({ ...p, status }));
      toast.success('Status updated');
    } catch (err) { toast.error('Failed to update status'); }
  };

  const sendContactReply = async () => {
    if (!selectedContactMsg || !contactReply.trim()) return;
    setSendingContactReply(true);
    try {
      await api.post(`/admin/contact-messages/${selectedContactMsg._id}/reply`, { reply: contactReply });
      toast.success('Reply sent successfully!');
      setContactReply('');
      setSelectedContactMsg((p: any) => ({ ...p, status: 'REPLIED', adminReply: contactReply }));
      fetchContactMessages();
    } catch (err: any) {
        ...
        {activeTab === 'contact_messages' && (
          <div className="space-y-6">
            ...
            <div className="flex gap-4 h-[600px]">
              {/* Left: Messages List */}
              <div className="w-full md:w-80 flex-shrink-0 bg-white rounded-2xl border border-gold/20 shadow-sm overflow-hidden flex flex-col">
                <div className="p-3 border-b border-gold/10 bg-cream/30">
                  <span className="text-xs font-bold text-primary/70 uppercase tracking-wider">Messages ({contactMessages.length})</span>
                </div>
                <div className="overflow-y-auto flex-1 divide-y divide-gold/10">
                  {contactMessages.length === 0 ? (
                    <div className="p-8 text-center text-foreground/40 text-xs">No messages yet.</div>
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        setContactReply('');
                        if (msg.status === 'UNREAD') await updateContactMessageStatus(msg._id, 'READ');
                      }}
                      className={`w-full text-left p-3 transition-all hover:bg-cream/30 flex flex-col gap-1 ${
                        selectedContactMsg?._id === msg._id ? 'bg-cream/50 border-l-4 border-primary' : ''
                      }`}
                    >
                      <div className="flex items-start justify-between gap-2">
                        <span className="text-xs font-bold text-primary truncate max-w-[120px]">{msg.name}</span>
                      ...
                      <span className="text-[10px] text-foreground/60 truncate">{msg.subject}</span>
                      <span className="text-[9px] text-foreground/40">{new Date(msg.createdAt).toLocaleDateString()}</span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="flex-1 bg-white rounded-2xl border border-gold/20 shadow-sm overflow-hidden flex flex-col">
                {selectedContactMsg ? (
                  <>
                    {/* Header */}
                    <div className="p-5 border-b border-gold/10 flex items-start justify-between gap-4">
                      <div>
                        <h3 className="font-serif font-bold text-primary text-base">{selectedContactMsg.subject}</h3>
                        <p className="text-xs text-foreground/50 mt-0.5">
                          From: <span className="font-semibold text-foreground/70">{selectedContactMsg.name}</span>
                          {' '}&lt;{selectedContactMsg.email}&gt;
                          {' · '}{new Date(selectedContactMsg.createdAt).toLocaleString()}
                        </p>
                      </div>
                      <div className="flex items-center gap-2 flex-shrink-0">
                        <select
                          value={selectedContactMsg.status}
                          onChange={(e) => updateContactMessageStatus(selectedContactMsg._id, e.target.value)}
                          className="border border-gold/30 rounded-xl px-3 py-1.5 text-xs font-semibold bg-cream focus:outline-none focus:border-primary cursor-pointer"
                        >
                    ...
                    <div className="flex-1 overflow-y-auto p-5 space-y-4">
                      <div className="bg-cream/40 rounded-2xl p-4">
                        <p className="text-xs font-bold text-foreground/50 uppercase tracking-wider mb-2">Message</p>
                        <p className="text-sm text-foreground/80 leading-relaxed whitespace-pre-wrap">{selectedContactMsg.message}</p>
                      </div>

                      {selectedContactMsg.adminReply && (
                        <div className="bg-primary/5 border border-primary/10 rounded-2xl p-4">
                          <p className="text-xs font-bold text-primary/60 uppercase tracking-wider mb-2">Your Previous Reply</p>
                          <p className="text-sm text-foreground/80 leading-relaxed whitespace-pre-wrap">{selectedContactMsg.adminReply}</p>
                        </div>
                      )}
                    </div>

                    {/* Reply Box */}
                    <div className="p-4 border-t border-gold/10 bg-cream/20">
                      <textarea
                        value={contactReply}
                        onChange={(e) => setContactReply(e.target.value)}
                        placeholder={`Reply to ${selectedContactMsg.name}...`}
                        rows={3}
                        className="w-full border border-gold/30 rounded-2xl px-4 py-3 text-sm bg-white focus:outline-none focus:border-primary resize-none"
                      />
                      <div className="flex justify-end mt-2">
                        <button
                          onClick={sendContactReply}
                          disabled={sendingContactReply || !contactReply.trim()}
                          className="flex items-center gap-2 px-5 py-2.5 bg-primary text-white rounded-full text-xs font-bold hover:bg-primary/90 disabled:opacity-50 transition-all"
                        >
                          {sendingContactReply ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Send className="w-3.5 h-3.5" />}
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center text-center gap-3 text-foreground/30 p-8">
                    <MessageSquare className="w-12 h-12 opacity-20" />
                    <p className="text-sm font-semibold">Select a message to view and reply</p>
                  </div>
                )}
              </div>
            </div>
