...
export default function EditSaleCampaignPage() {
  ...
  useEffect(() => {
    const loadDependenciesAndCampaign = async () => {
      try {
        const [catRes, svcRes, campRes] = await Promise.all([
          api.get('/admin/categories'),
          api.get('/admin/services?limit=1000'),
          api.get(`/admin/sale-campaigns/${id}`)
        ]);
        ...
        setForm({
          ...
          applicableServices: (c.applicableServices || []).map((svc: any) => svc._id || svc),
        });
      } catch (err: any) {
    ...
    if (id) loadDependenciesAndCampaign();
  }, [id]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.put(`/admin/sale-campaigns/${id}`, payload);
      setSuccess('Sale campaign updated successfully!');
      setTimeout(() => router.push('/admin/dashboard?tab=campaigns'), 1200);
    } catch (err: any) {
      setError(err.response?.data?.message || 'Failed to update sale campaign.');
  ...
  if (loading) {
    return (
      <AdminPageLayout title="Edit Sale Campaign" backHref="/admin/dashboard?tab=campaigns" backLabel="Back to Campaigns">
  ...
  return (
    <AdminPageLayout title={`Edit Campaign: ${form.name}`} subtitle="Modify sale event configurations" backHref="/admin/dashboard?tab=campaigns" backLabel="Back to Campaigns">
        ...
        <div className="bg-white rounded-3xl border border-[#C3AB84]/20 shadow-sm p-6 sm:p-8 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div>
              ...
              <input required type="number" min={0} max={100} value={form.discountPercentage} onChange={e => setForm(p => ({ ...p, discountPercentage: e.target.value }))} className={inp} />
            </div>
            <div />
